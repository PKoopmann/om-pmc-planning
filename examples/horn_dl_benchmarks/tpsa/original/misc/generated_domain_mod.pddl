...
(:action billingActivation
  ...
  :precondition (and (DATALOG_ACTIVATION ?x) (not (= ?x ?y)) (not (DATALOG_INCONSISTENT)))
  :effect (and (invoice ?y)))
(:action confirmOrder
  :parameters (?x ?y ?z - object)
  :precondition (and (DATALOG_SERVICE ?y) (order ?x) (requestedservice ?x ?y) (not (= ?x ?y)) (not (= ?x ?z)) (not (= ?y ?z)) (not (DATALOG_INCONSISTENT)))
  :effect (and (hasservice ?z ?y) (orderconfirmation ?z)))
(:action contractSaving
  :parameters (?x ?y ?z - object)
  :precondition (and (DATALOG_CONTRACT ?y) (courierack ?x) (not (= ?x ?y)) (not (= ?x ?z)) (not (= ?y ?z)) (not (DATALOG_INCONSISTENT)))
  :effect (and (crmack ?z)))
(:action courierService
  :parameters (?x ?y - object)
  :precondition (and (DATALOG_CONTRACT ?x) (not (= ?x ?y)) (not (DATALOG_INCONSISTENT)))
  :effect (and (courierack ?y)))
(:action prepareContract
  :parameters (?x ?y ?z - object)
  :precondition (and (DATALOG_SERVICE ?y) (hasservice ?x ?y) (orderconfirmation ?x) (not (= ?x ?y)) (not (= ?x ?z)) (not (= ?y ?z)) (not (DATALOG_INCONSISTENT)))
  :effect (and (requiredservice ?z ?y) (contract ?z)))
(:action prepareHardware
  :parameters (?x ?y ?z - object)
  :precondition (and (DATALOG_CONFIRMATION ?x) (DATALOG_SERVICE ?y) (hasservice ?x ?y) (not (= ?x ?y)) (not (= ?x ?z)) (not (= ?y ?z)) (not (DATALOG_INCONSISTENT)))
  :effect (and (forservice ?z ?y) (hardwareack ?z)))
(:action productActivation
  :parameters (?x ?y ?z - object)
  :precondition (and (crmack ?x) (hardwareack ?y) (not (= ?x ?y)) (not (= ?x ?z)) (not (= ?y ?z)) (not (DATALOG_INCONSISTENT)))
